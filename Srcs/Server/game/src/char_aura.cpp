...
void CHARACTER::AuraRefineWindowCheckIn(BYTE bAuraRefineWindowType, TItemPos AuraCell, TItemPos ItemCell)
{
	...
	if (!IsAuraRefineWindowOpen())
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1972]");		return;
	}
	...
	if (!IsAuraRefineWindowCanRefine())
	{
		...
		{
			ChatPacket (CHAT_TYPE_INFO, "[LS;1973]");		return;
		}
	}

	if (m_bAuraRefineWindowType != bAuraRefineWindowType)
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1974]");		return;
	}
	...
	switch (bAuraRefineWindowType)
	{
		case AURA_WINDOW_TYPE_ABSORB:
		{
			...
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1975]");		return;
			}

			if (ItemCell.IsEquipPosition())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1976]");		return;
			}

			LPITEM pItem = GetItem(ItemCell);
			if (!pItem)
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1977]");		return;
			}
			...
			*/
			...
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1981]");		return;
			}
...
/*#if defined(__BL_TRANSMUTATION__)
			sub.pItem.dwTransmutationVnum = pItem->GetVnum();
#endif*/
...
/*#if defined(__ITEM_APPLY_RANDOM__)
		...
		case AURA_WINDOW_TYPE_GROWTH:
		{
			if (!ItemCell.IsValidItemPosition())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1982]");		return;
			}

			if (ItemCell.IsEquipPosition())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1983]");		return;
			}
			...
			*/
			...
			if (pItem->isLocked())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1984]");		return;
			}
			...
			if (AuraCell.cell == AURA_SLOT_MAIN)
			{
				...
				else
				{
					...
					if (c_bCurrentLevel == aiAuraRefineTable[AURA_REFINE_INFO_LEVEL_MAX] && c_wCurrentExp == aiAuraRefineTable[AURA_REFINE_INFO_NEED_EXP])
					{
						...
						if (aiAuraRefineTable[AURA_REFINE_INFO_STEP] == AURA_GRADE_RADIANT)
							ChatPacket (CHAT_TYPE_INFO, "[LS;1986]");
						else
		...
		case AURA_WINDOW_TYPE_EVOLVE:
		{
			...
			if (ItemCell.IsEquipPosition())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1991]");
				return;
			}
			...
			if (pItem->isLocked())
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;1992]");
				return;
			}
			...
			if (AuraCell.cell == AURA_SLOT_MAIN)
			{
				...
				else
				{
					...
					if (!(c_bCurrentLevel == aiAuraRefineTable[AURA_REFINE_INFO_LEVEL_MAX] && c_wCurrentExp == aiAuraRefineTable[AURA_REFINE_INFO_NEED_EXP]))
					{
						ChatPacket (CHAT_TYPE_INFO, "[LS;1993]");
						ChatPacket (CHAT_TYPE_INFO, "%d[LS;1994]%d"), aiAuraRefineTable[AURA_REFINE_INFO_LEVEL_MAX], aiAuraRefineTable[AURA_REFINE_INFO_NEED_EXP];
						return;
					}
				}
			}
			...
			if (AuraCell.cell == AURA_SLOT_SUB)
			{
				if (NPOS == m_pAuraRefineWindowItemSlot[AURA_SLOT_MAIN])
				{
					ChatPacket (CHAT_TYPE_INFO, "[LS;1988]");
					return;
				}
				else
				{
					...
					{
						ChatPacket (CHAT_TYPE_INFO, "[LS;1995]");
						TItemTable* pTable = ITEM_MANAGER::instance().GetTable(aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_VNUM]);
						if (pTable)
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Hangisi, %s - %u"), pTable->szLocaleName, aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_COUNT]);
						return;
					}
				}
			}
...
void CHARACTER::AuraRefineWindowCheckOut(BYTE bAuraRefineWindowType, TItemPos AuraCell)
{
	...
	if (!IsAuraRefineWindowOpen())
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1996]");
		return;
	}
...
void CHARACTER::AuraRefineWindowAccept(BYTE bAuraRefineWindowType)
{
	...
	if (!IsAuraRefineWindowCanRefine())
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<aura> ??leme devam etmek icin tum pencereleri kapatın"));
			return;
		}
	}
	...
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1997]");
		return;
	}
	...
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1998]");
		return;
	}
	...
	{
		ChatPacket (CHAT_TYPE_INFO, "[LS;1999]");
		return;
	}
#endif
	switch (bAuraRefineWindowType)
	{
		case AURA_WINDOW_TYPE_ABSORB:
		{
			ChatPacket (CHAT_TYPE_INFO, "[LS;2000]");
		...
		case AURA_WINDOW_TYPE_GROWTH:
		{
			...
			ChatPacket (CHAT_TYPE_INFO, "[LS;2001]");
		...
		case AURA_WINDOW_TYPE_EVOLVE:
		{
			...
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;2002]");
				if (aiAuraRefineTable[AURA_REFINE_INFO_STEP] == AURA_GRADE_RADIANT)
					ChatPacket (CHAT_TYPE_INFO, "[LS;1986]");
				else
					ChatPacket (CHAT_TYPE_INFO, "[LS;2003]");
				return;
			}
			...
			if (number(1, 100) <= c_bRefinePct)
			{
				...
				if (pkNewItem)
				{
					ChatPacket (CHAT_TYPE_INFO, "[LS;2006]");
			...
			{
				ChatPacket (CHAT_TYPE_INFO, "[LS;2007]");
