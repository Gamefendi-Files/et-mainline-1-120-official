...
int CItem::FindEquipCell (LPCHARACTER ch, int iCandidateCell)
{
	...
	else if (GetType() == ITEM_COSTUME)
	{
		...
		return ch->GetEquipIndex() == FIRST_TYPE_EQUIPMENT ? WEAR_PENDANT : WEAR_SECOND_PENDANT;
#endif
#if defined(__GLOVE_SYSTEM__)
	else if (GetWearFlag() & WEARABLE_GLOVE)
		return ch->GetEquipIndex() == FIRST_TYPE_EQUIPMENT ? WEAR_GLOVE : WEAR_SECOND_GLOVE;
#endif
#endif
...
void CItem::ModifyPoints (bool bAdd)
{
		...
		if (m_pProto->bType == ITEM_WEAPON || m_pProto->bType == ITEM_ARMOR)
		{
			...
			for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
			{
				...
				if (ITEM_METIN == p->bType)
				{
					...
					{
						if (p->aApplies[i].wType == APPLY_NONE)
						{
							continue;
						}
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
						...
						{
							m_pOwner->ApplyPoint (p->aApplies[i].wType, bAdd ? p->aApplies[i].lValue : p->aApplies[i].lValue ^ 0x00800000);
						}
						else
						{
							m_pOwner->ApplyPoint (p->aApplies[i].wType, bAdd ? p->aApplies[i].lValue : -p->aApplies[i].lValue);
						}
					}
				}
			}
	...
	{
#ifdef ENABLE_ACCE_SYSTEM
		if ((m_pProto->aApplies[i].wType == APPLY_NONE) && (GetType() != ITEM_COSTUME) && (GetSubType() != COSTUME_ACCE))
#else
		...
		case ITEM_COSTUME:
			{
				...
				else if (GetSubType() == COSTUME_HAIR)
				{
					...
					toSetValue = (true == bAdd) ? this->GetValue(3) : 0;
#endif
					if (false == bAdd) // Kostum cıkarılma durumu
					{
						toSetValue = (true == bAdd) ? this->GetValue(3) : 0;
					}
				}
...
long CItem::FindApplyValue (BYTE bApplyType)
{
	...
	for (int i = 0; i < ITEM_APPLY_MAX_NUM; ++i)
	{
		if (m_pProto->aApplies[i].wType == bApplyType)
		{
