	...
	if (uiCountSec % 60 == 0)
	{
		auto FExitNotice = [&uiCountSec](LPENTITY lpEntity)
		{
			...
			if (c_lpChar && c_lpChar->IsPC())
				//c_lpChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Acele etme! Ana karaya ulaþmak ve portaldan geçmek için %d dakikanýz var."), (uiCountSec % 3600) / 60);
				c_lpChar->ChatPacket (CHAT_TYPE_INFO, "[LS;2072;%d]", (uiCountSec % 3600) / 60);
		};
	...
	case ShipDefense::WAVE4:
	{
		auto FWaveNotice = [&bNextWave, &byWave, &byWaitCount](LPENTITY lpEntity)
		{
			...
			if (c_lpChar && c_lpChar->IsPC())
			{
				...
				{
					if (bNextWave == true)
						//c_lpChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Sonraki dalga %d saniye içinde baþlýyor."), byWaitCount);
						c_lpChar->ChatPacket (CHAT_TYPE_INFO, "[LS;2073;%d]", byWaitCount);
					else
					//	c_lpChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Deniz savaþý %d saniye içinde baþlýyor."), byWaitCount);
						c_lpChar->ChatPacket (CHAT_TYPE_INFO, "[LS;2074;%d]", byWaitCount);
				}
				break;
				}
			}
				...
				if (it != LaserEffectDataMap.end())
				{
					...
					if (lXCharPos == it->second.x && lYCharPos == it->second.y)
					{
						if (c_lpUniqueChar->FindAffect(AFFECT_DEFENSEWAVE_LASER))
						{
							...
							{
								if (test_server)
									//c_lpChar->ChatPacket(CHAT_TYPE_INFO, "<TEST_SERVER> Bu etkiyi %d saniye boyunca kaldýramazsýnýz.", ShipDefense::EFFECT_SHELTER_DURATION);
									c_lpChar->ChatPacket (CHAT_TYPE_INFO, "[LS;2075;%d]", ShipDefense::EFFECT_SHELTER_DURATION);
								return;
							}
...
void CShipDefense::NoticeByType(const ShipDefense::ENoticeType c_eType)
{
	...
	auto FNotice = [this, &c_eType](LPENTITY lpEntity)
	{
		...
		if (c_lpChar && c_lpChar->IsPC())
		{
			...
			//	Notice(c_lpChar, true, "Deniz canavarlarýnýn saldýrýsý altýndayýz! Savaþmaya hazýr olun!");
			//	Notice(c_lpChar, true, "%d dalgasý: %d saniye için. %s'i tüm gücünüzle korumalýsýnýz.",
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2077]");
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2076;%d;%d;%s]",
			...
			//	Notice(c_lpChar, true, "Yine saldýrýyorlar! Silahlarýnýzý hazýrlayýn!");
			//	Notice(c_lpChar, true, "Dalga %d: Korunurken %s, Yenmek %s.",
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2078]");
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2079;%d;%s;%s]",
					GetWave() + 1, c_lpAllianceChar->GetName(), c_lpHydraChar->GetName());
			}
			break;

			case NOTICE_WAVES:
			//	Notice(c_lpChar, true, "Kara görüldü! Uzun ve yorucu bir yolculuðun ardýndan direði baþarýyla koruyarak yeni kýtaya ulaþtýnýz. Portala devam edin.");
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2080]");
				break;
			...
			case NOTICE_MAST_HP:
			{
				...
				//Notice(c_lpChar, false, "%s's Kalan HP: %d%%",
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2081;%s;%d%%]",
					c_lpAllianceChar->GetName(), c_lpAllianceChar->GetHPPct());
			}
			break;

			case NOTICE_MAST_PROTECTED:
			//	Notice(c_lpChar, true, "Direði baþarýyla korudunuz. Aynen böyle devam! Bir sonraki dalgaya hazýr olun.");
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2082]");
				break;

			case NOTICE_MAST_DESTROYED:
			//	Notice(c_lpChar, true, "Direk parçalandý ve geminiz batýyor. Artýk Cape Dragon Fire'a ýþýnlanacaksýnýz.");
				c_lpChar->ChatPacket (CHAT_TYPE_NOTICE, "[LS;2083]");
				break;
			}
		}
	};
...
bool CShipDefenseManager::Create(const LPCHARACTER c_lpChar)
{
	...
	if (!lPrivateMapIndex)
	{
	//	c_lpChar->ChatPacket(CHAT_TYPE_INFO, "Özel harita oluþturulamadý.");
		c_lpChar->ChatPacket (CHAT_TYPE_INFO, "[LS;2084;]");
		return false;
	}
	...
	if (pShipDefense != nullptr)
	{
		...
		//	c_lpParty->ChatPacketToAllMember(CHAT_TYPE_NOTICE, LC_TEXT("'Gemi Savunmasý' zindaný hazýr. Balýkçý þimdi gemiye binmene izin verecek."));
			c_lpParty->ChatPacketToAllMember (CHAT_TYPE_NOTICE, "[LS;2085;]");
		}
