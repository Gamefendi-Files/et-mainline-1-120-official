...
bool g_bChangeCostumeAttributes = false;
bool g_bResetCostumeAttributes = false;
...
void config_init (const string& st_localeServiceName)
{
	...
	*log_db = '\0';
	...
	while (fgets (buf, 256, fp))
	{
		...
		TOKEN("change_costume_attributes")
		{
			str_to_number(g_bChangeCostumeAttributes, value_string);
			continue;
		}

		TOKEN("reset_costume_attributes")
		{
			str_to_number(g_bResetCostumeAttributes, value_string);
			continue;
		}
