...
bool ITEM_MANAGER::ReadMonsterDropItemGroup (const char* c_pszFileName)
{
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		else if (strType == "thiefglove")
		{
			CBuyerThiefGloveItemGroup* pkGroup = M2_NEW CBuyerThiefGloveItemGroup (0, iMobVnum, stName);
			...
			m_map_pkGloveItemGroup.insert (std::map<DWORD, CBuyerThiefGloveItemGroup*>::value_type (iMobVnum, pkGroup));
		}
		else
		{
