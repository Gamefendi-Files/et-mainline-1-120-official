	...
	for (DWORD i = 0; i < dwCount; ++i, ++p)
	{
		...
		if (item->GetType() == ITEM_PET)
		{
			switch (item->GetSubType())
			{
				...
				case PET_UPBRINGING:
				{
					const long = (item->GetSocket(0) - get_global_time()) / 60;
					if (lPetTime > 0)
					{

					if ((p->window == INVENTORY) && (item->GetSocket(3) == TRUE))
						ch->PetSummon(item);
...
#if defined(__PET_SYSTEM__) && defined(ENABLE_PET_SUMMON_AFTER_LOGIN)
				case PET_PAY:
				{
					const long PetTime = (item->GetSocket(0) - get_global_time()) / 60;
					if (PetTime > 0)
					{
						if ((p->window == INVENTORY) && (item->GetSocket(1) == TRUE))
							ch->SetPetPay(item);
					}
				}
				break;
