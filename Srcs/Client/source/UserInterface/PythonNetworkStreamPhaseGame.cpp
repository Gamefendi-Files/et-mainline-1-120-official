	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			if (pInstTarget->IsBuilding()
				...
				#if defined(VIEW_TARGET_DECIMAL_HP)
				, TargetPacket.iMinHP, TargetPacket.iMaxHP
				#endif
				));
...
bool CPythonNetworkStream::RecvRefineInformationPacketNew()
{
	...
	for (BYTE bApplySlot = 0; bApplySlot < ITEM_APPLY_RANDOM_SLOT_MAX_NUM; ++bApplySlot)
		PyList_Append(poList, Py_BuildValue("(ii)", kRefineInfoPacket.aApplyRandom[bApplySlot].bType, kRefineInfoPacket.aApplyRandom[bApplySlot].sValue));
#endif
...
/*#if defined(__BL_TRANSMUTATION__)
			kItemData.dwTransmutationVnum = kSubPacket.pItem.vnum;
#endif*/
